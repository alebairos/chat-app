  ...
  try {
    ...
    if (totalActivities > 0) {
      ...
      for (final activity in todayActivities) {
        final time = '${activity.completedAt.hour.toString().padLeft(2, '0')}:${activity.completedAt.minute.toString().padLeft(2, '0')}';
        print('  • ${activity.activityCode} (${activity.activityName}) at $time');
      }
      ...
      for (final activity in recentActivities) {
        final date = '${activity.completedAt.year}-${activity.completedAt.month.toString().padLeft(2, '0')}-${activity.completedAt.day.toString().padLeft(2, '0')}';
        byDate[date] ??= [];
      ...
      for (final activity in activities.take(5)) {
        print('  • ${activity['code']}: ${activity['name']} at ${activity['time']}');
      }
      
      final summary = statsToday['summary'];
      print('Summary:');
      print('  - By dimension: ${summary['by_dimension']}');
      print('  - Most frequent: ${summary['most_frequent']} (${summary['max_frequency']}x)');
    }
