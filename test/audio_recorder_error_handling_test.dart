...
void main() {
  ...
  testWidgets('error messages have consistent screen margins',
      (WidgetTester tester) async {
    debugPrint('\n=== STARTING MARGIN TEST ===');

    // Use a fixed screen size for consistent testing
    tester.binding.window.physicalSizeTestValue = const Size(800, 600);
    tester.binding.window.devicePixelRatioTestValue = 1.0;

    debugPrint('Test configuration:');
    debugPrint('Screen size: ${tester.binding.window.physicalSize}');
    debugPrint('Device pixel ratio: ${tester.binding.window.devicePixelRatio}');
        ...
        home: Scaffold(
          body: Builder(
            builder: (context) {
              ...
              debugPrint('\nMediaQuery data:');
              debugPrint('Size: ${mediaQuery.size}');
    ...
    debugPrint('\nAfter first pump:');
    debugPrint(
        'Found Banner: ${find.byType(MaterialBanner).evaluate().length > 0}');
    ...
    debugPrint('\nBanner measurements:');
    debugPrint('Left: ${bannerRect.left}');
    debugPrint('Right: ${bannerRect.right}');
    debugPrint('Width: ${bannerRect.width}');
    debugPrint('Screen width: ${screenSize.width}');
    debugPrint(
        'Screen width in logical pixels: ${screenSize.width / devicePixelRatio}');

    // Verify banner is at the top of the screen
    expect(bannerRect.top, 0.0,
        reason: 'Banner should be at the top of the screen');
    expect(bannerRect.height, greaterThan(0.0),
        reason: 'Banner should have a positive height');
  });

  testWidgets('error messages have consistent maximum width and centering',
      (WidgetTester tester) async {
    // Use a wide screen size
    tester.binding.window.physicalSizeTestValue = const Size(800, 600);
    tester.binding.window.devicePixelRatioTestValue = 1.0;
    ...
    // Get banner position and screen size
    final bannerRect = tester.getRect(find.byType(MaterialBanner));
    final screenSize = tester.binding.window.physicalSize;

    // Verify banner spans the full width
    expect(bannerRect.width, screenSize.width,
        reason: 'Banner should span the full width of the screen');
  });
  ...
  testWidgets('error messages can be dismissed by tap',
      (WidgetTester tester) async {
          ...
          body: Builder(
            builder: (context) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                ...
                final snackBarWidth = screenWidth - 30.0;
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    ...
                    margin: EdgeInsets.fromLTRB(
                      (screenWidth - (snackBarWidth.clamp(0.0, maxWidth))) / 2,
                      0,
                      (screenWidth - (snackBarWidth.clamp(0.0, maxWidth))) / 2,
                      10.0,
                    ),
