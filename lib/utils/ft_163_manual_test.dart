...
import '../services/activity_memory_service.dart'
    hide ActivityQueue; // Hide FT-119 ActivityQueue to avoid conflict
...
class FT163ManualTest {
  ...
  static Future<void> testSingleActivity() async {
    ...
    try {
      ...
      _logger
          .info('📥 Queued activity. Queue size: ${ActivityQueue.queueSize}');
      ...
      if (finalCount > initialCount) {
        ...
        final recentActivities =
            await ActivityMemoryService.getRecentActivities(1);
        if (recentActivities.isNotEmpty) {
          final savedActivity = recentActivities.first;
          _logger.info(
              '💾 Saved activity: ${savedActivity.activityCode} - ${savedActivity.activityName}');
          _logger.info('🏷️ Source: ${savedActivity.source}');
          _logger.info('📈 Confidence: ${savedActivity.confidenceScore}');
        }
      } else {
        _logger.warning(
            '⚠️ No activity was detected/saved. Check Oracle configuration.');
      }
  ...
  static Future<void> testMultipleActivities() async {
    ...
    try {
      ...
      for (final activity in activities) {
        await ActivityQueue.queueActivity(
            activity,
      ...
      _logger.info(
          '📥 Queued ${activities.length} activities. Queue size: ${ActivityQueue.queueSize}');
      ...
      _logger
          .info('📥 Queue size after processing: ${ActivityQueue.queueSize}');
      ...
      if (activitiesDetected > 0) {
        ...
        final recentActivities =
            await ActivityMemoryService.getRecentActivities(activitiesDetected);
        for (final activity in recentActivities) {
          _logger.info(
              '💾 ${activity.activityCode} - ${activity.activityName} (${activity.source})');
        }
      } else {
  ...
  static void showQueueStatus() {
    ...
    if (!ActivityQueue.isEmpty) {
      ...
      _logger.info(
          '  - ${ActivityQueue.queueSize} activities waiting to be processed');
    }
  }
  ...
  static Future<void> simulateRateLimitScenario() async {
    ...
    try {
      ...
      await ActivityQueue.queueActivity(
          'Bebi água durante rate limit', DateTime.now());
      await ActivityQueue.queueActivity(
          'Fiz exercício durante rate limit', DateTime.now());

      _logger.info(
          '📥 Activities queued during "rate limit": ${ActivityQueue.queueSize}');
